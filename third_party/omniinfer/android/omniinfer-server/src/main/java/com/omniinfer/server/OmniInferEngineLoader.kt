 ...
 * runtime instead of being bundled inside the host APK.
 *
 ...
 * libraries cannot be safely unloaded once ggml backends have registered.
 *
 * A successful [install] also points [OmniInferNativeLibs.installedNativeLibDir] at
 * the engine's `lib/<abi>` folder, so every later model load resolves ggml
 * backends from the engine payload instead of `applicationInfo.nativeLibraryDir`.
 */
object OmniInferEngineLoader {
    ...
    @Volatile
    private var installedDir: File? = null

    @Volatile
    private var installedManifest: OmniInferEngineManifest? = null

    /**
     * Native lib dir to hand to the JNI backend once an engine package is loaded,
     * or `null` when no engine package is installed (the host then falls back to a
     * filtered mirror of `applicationInfo.nativeLibraryDir`).
     */
    ...
    fun installedManifest(): OmniInferEngineManifest? = installedManifest

    fun installedEngineDir(): File? = installedDir
    ...
    fun verify(engineDir: File, verifyHashes: Boolean = true): OmniInferEngineManifest {
        ...
        if (!libDir.isDirectory) {
            throw OmniInferEngineException(
                "Engine package is missing ${manifest.libDirRelativePath}: ${engineDir.absolutePath}",
            )
        }
        ...
        for (entry in manifest.libs.values) {
            ...
            if (!file.isFile) {
                throw OmniInferEngineException(
                    "Engine package is missing lib '${entry.name}'. Re-download the engine package.",
                )
            }
            if (file.length() != entry.sizeBytes) {
                throw OmniInferEngineException(
                    "Engine lib '${entry.name}' has unexpected size ${file.length()} " +
                        "(expected ${entry.sizeBytes}). Re-download the engine package.",
                )
            }
            if (verifyHashes && file.sha256Hex() != entry.sha256) {
                throw OmniInferEngineException(
                    "Engine lib '${entry.name}' failed SHA-256 verification. Re-download the engine package.",
                )
            }
        }

        // Anything that ggml could dlopen must be accounted for by the manifest.
     ...
     * process. Afterwards [OmniInferNativeLibs] resolves through the engine package,
     * so ggml discovers its backends from the same directory.
     *
    ...
    fun install(engineDir: File, verifyHashes: Boolean = true): OmniInferEngineManifest {
        installedDir?.let { existing ->
            ...
            throw OmniInferEngineException(
                "OmniInfer engine ${installedManifest?.engineVersion ?: "?"} is already installed " +
                    "in this process from $existing. Restart the app to install a different engine package.",
            )
        }
        ...
        synchronized(this) {
            ...
            OmniInferNativeLibs.installedNativeLibDir = libDir.absolutePath
        }
        OmniInferBridge.markEngineNativeLibsLoaded()
        Log.i(
            TAG,
            "OmniInfer engine ${manifest.engineVersion} installed " +
                "(${manifest.backends}) from ${engineDir.absolutePath}",
        )
        return manifest
    }
